...
class LocalModelEvaluator:
    ...
    def process_entry(self, question, expected_answer) -> Dict[str, Any]:
        """
        ...
        for _ in range(self.config.eval_repeats):
            try:
                raw_response = self.get_model_response(question)
                model_answer = extract_answer(raw_response)
                score = int(str(model_answer) == str(expected_answer))
    ...
    def evaluate_dataset(self, category_name: str, dataset_config: DatasetConfig) -> Dict[str, Any]:
        ...
        results = []
        for question, answer in tqdm(zip(questions, answers), total=len(questions), desc=f"Processing {dataset_name}"):
            results.append(self.process_entry(question, answer))
