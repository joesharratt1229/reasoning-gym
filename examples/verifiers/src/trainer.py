...
from transformers import GenerationConfig, PreTrainedModel, PreTrainedTokenizerBase, Trainer
...
class GRPOVerifierTrainer(GRPOTrainer):
        ...
        if self.args.use_vllm:
            ...
            if self.accelerator.is_main_process:
                if self.env is not None:
                    outputs = self.env.generate(prompts=all_prompts, llm=self.llm, sampling_params=self.sampling_params)
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        attention_mask = torch.cat([prompt_inputs["attention_mask"], completion_mask], dim=1)  # (B*G, P+C)
